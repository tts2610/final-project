...
export default function MenuCarousel({ category, restaurant }) {
  ...
  return (
    <Row className="menu-row" style={{ display: "flex", backgroundColor: "#f7f7f7", padding: "20px" }}>
      {menusByID &&
        menusByID.map((item) => (
          <Col className="mr-2 mt-2" style={{ display: "flex", padding: "10px", backgroundColor: "white", borderRadius: "5px", minWidth: "350px", alignItems: "center" }}>
            <Image className="food mr-2" fluid width="100px" height="100px" src={item.image} alt=""></Image>
            <div>{item.title}</div>
          </Col>
        ))}
    </Row>
