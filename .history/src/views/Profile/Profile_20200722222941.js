...
export default function Profile() {
  ...
  const activitiesParams = useSelector((state) => state.activitiesParams);
  const dispatch = useDispatch();
  useEffect(() => {
    async function getUser() {
      const user = await fetchUser();
      if (user) setUser(user.data);
      const { pagination, result } = await getActivities(user.data._id, activitiesParams.page);
  ...
  const handlePageClick = (e) => {
    ...
    dispatch({ type: "ACTIVITY", payload: { activitiesParams: { page: clickValue } } });
  };
