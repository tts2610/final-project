...
export default function Profile() {
  ...
  const activityParams = useSelector((state) => state.activityParams);
  const dispatch = useDispatch();
  useEffect(() => {
    async function getUser() {
      const user = await fetchUser();
      if (user) setUser(user.data);
      const { pagination, result } = await getActivities(user.data._id, activityParams.page);
      ...
      <div class="container">
        <div class="col-lg-8" style={{ display: "contents" }}>
          ...
          <div class="panel">
            ...
            <CustomPagination handlePageClick={handlePageClick} maxPages={totalPage} active={activityParams.page} />
          </div>
          {user.role !== "owner" && (
            <div class="panel">
              <div class="panel-heading">
