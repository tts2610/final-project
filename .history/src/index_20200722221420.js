...
function reducer(state = initialState, action) {
  if (action.type === "LOGINMODAL") {
    ...
    let newParams = { ...state.activitiesParams };
    for (const [key, value] of Object.entries(action.payload.activitiesParams)) {
      newParams[key] = value;
    }
    state.activitiesParams = { ...newParams };
  }
  state = { ...state };
  return state;
}
