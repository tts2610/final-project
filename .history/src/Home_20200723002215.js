...
export default function Home() {
  ...
  const performFilter = (params) => {
    ...
    let searchUrl = `${process.env.REACT_APP_API_URL}/restaurants/search?tags=${params.tags}&averageRatingMin=${params.averageRating ? params.averageRating : ""}&page=${params.page}&&perPage=12`;
  ...
  useEffect(() => {
    ...
    async function getRes() {
      ...
      axios
        .get(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${localStorage.getItem("currentLat")},${localStorage.getItem("currentLng")}&key=${process.env.REACT_APP_GOOGLEMAP}`)
  ...
  return (
    <div className="page">
      <div>
        <MyMap />
        <Image onClick={() => clickScroll()} width="100" height="100" style={{ cursor: "pointer", position: "absolute", zIndex: "10", top: "90%", left: "50%" }} src="https://media.giphy.com/media/Wtg8Bmgul1Qxc0otod/giphy.gif" alt="" />
        <div style={{ position: "absolute", zIndex: "10", top: "100%", left: "50%" }} ref={ref}></div>
      </div>

      <Header />

      <h1 className="py-4" style={{ fontFamily: "Courgette, cursive", textAlign: "center" }}>
