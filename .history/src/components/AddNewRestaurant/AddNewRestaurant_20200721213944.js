  ...
  const handleInputChange = (e, index) => {
    ...
    if (files) {
      ...
      for (const key of Object.keys(files)) {
        list[index][name] = JSON.stringify(files[key]);
      }
    ...
    <ReactCardFlip isFlipped={isFlipped} flipDirection="horizontal">
      ...
      <div>
        {inputList.map((x, i) => {
          return (
            <div className="box">
              <input name="title" placeholder="Enter First Name" value={x.title} onChange={(e) => handleInputChange(e, i)} />
              <input type="file" className="ml10" name="image" value={x.image} onChange={(e) => handleInputChange(e, i)} />
