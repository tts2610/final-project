...
export default function AddNewRestaurant() {
  ...
  const insertRestaurant = () => {
    ...
    axios
      .post(`${process.env.REACT_APP_API_URL}/restaurants/`, formData)
      .then((res) => {
        const { status } = res;
        if (status === 200) {
          ...
          filterList.forEach((element) => {
            ...
            axios
              .post(`${process.env.REACT_APP_API_URL}/menu/`, formDataMenu)
        ...
        setshowAddLoader(false);
        toast.success("Add new restaurant successfully!");
        window.location.reload();
      })
      .catch((err) => {
        alert("Opps! Unauthorized");
        history.replace("/");
      });
  };
      ...
      <ReactCardFlip isFlipped={isFlipped} flipDirection="horizontal">
        ...
        <div>
          {inputList.map((x, i) => {
            return (
              <div className="box">
                ...
                <input className="ml-3" name="category" placeholder="Enter Food Category" value={x.category} onChange={(e) => handleInputChange(e, i)} />
          ...
          <Button className="mt-5 ml-3" variant="success" onClick={insertRestaurant}>
            Add new!{showAddLoader && <Spinner size="sm" animation="border" className="ml-2" variant="light" />}
          </Button>
        </div>
      </ReactCardFlip>
      <ToastContainer />
