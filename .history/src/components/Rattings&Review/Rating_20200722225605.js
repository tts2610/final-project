...
export default function Rating({ restaurant }) {
  ...
  const setInterted = (e) => {
    setIsInsertedReview(e);
  };

  useEffect(() => {
    async function getReview() {
      const reviews = await getReivewByRestaurantID(restaurant._id, reviewsParams.page);
      // console.log(reviews);
      setReview(reviews.reviewList);
    }
    getReview();
  }, [isInsertedReview, restaurant._id, reviewsParams]);
  useEffect(() => {
  ...
  const handlePageClick = (e) => {
    ...
    dispatch({ type: "REVIEWS", payload: { reviewsParams: { page: clickValue } } });
  };
  return (
    <div>
        ...
        <div className="row justify-content-center">
          <div className="col-lg-12 col-md-12" style={{ paddingLeft: "0px", paddingRight: "0px" }}>
            <div className="card border-0 px-3">
              ...
              <CustomPagination handlePageClick={handlePageClick} maxPages={totalPage} active={activitiesParams.page} />

              {user && <Review user={user} setInterted={setInterted} restaurantID={restaurant._id} />}
              <div ref={ref}></div>
              <ToastContainer />
            </div>
          </div>
